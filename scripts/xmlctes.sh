...
'
  ...
  # instead of printing the code point, we print the offset on xmlp_entitties_map
...
hash_str () {
  ...
  if [ $i1 -ge $AA ]; then if [ $i1 -ge $aa ]; then i1=$((2 + ((i1 - aa + 3) >> 4))); else i1=$((1 + ((i1 - AA + 3) >> 4))); fi; else if [ $i1 -ge $d0 ]; then 1; else i1=0; fi; fi
  i0=$( LC_CTYPE=C printf '%d' "'${1:0:1}" )
  if [ $i0 -ge $aa ]; then i0=$((((i0 - aa + 27) << 2) + i1)) ; else i0=$((((i0 - AA + 1) << 2) + i1)) ; fi
  return $i0
}
